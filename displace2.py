...
if __name__ == "__main__":
    ...
    sim_interpol[(l//2)-np.int(inflectionpoint*resolution):(l//2)+np.int(inflectionpoint*resolution),0] = 0
    
    rbfi = Rbf(np.arange(l)+1, sim_interpol[:,0], epsilon=0.1)
    ...
    xi1 = np.linspace((l//2)-(interpolation_cut+inflectionpoint)*resolution, (l//2)+(interpolation_cut+inflectionpoint)*resolution, l)
    ...
    for n in range(num_steps):
        ...
        #left = sim_interpol[:(l//2)-np.int(inflectionpoint*resolution),0]
        #right = sim_interpol[(l//2)+np.int(inflectionpoint*resolution):,0]
        
        #y = np.concatenate((left,right))
        
        #rbfi = Rbf(np.arange(l)+1, sim_interpol[:,0], epsilon=0.1)
        rbfi = Rbf(np.arange(l)+1, sim_interpol[:,0], epsilon=0.1)
        #oldx = np.concatenate((1+np.arange(len(left)),np.int(inflectionpoint*resolution)+np.arange(len(right))))
        #rbfi = Rbf(oldx, y)
